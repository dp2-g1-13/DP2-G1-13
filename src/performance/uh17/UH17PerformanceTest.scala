...
class UH17PerformanceTest extends Simulation {
	...
	object RequestList {
		val requestList = exec(http("RequestList")
			.get("/flats/1/requests/list")
			.headers(headers_0))
		.pause(7)
	}

	
	object RequestListForbbiden {
		val requestListForbbiden = exec(http("RequestListForbbiden")
			.get("/flats/1/requests/list")
			.headers(headers_0)
			.check(status.is(403)))
		.pause(7)
	...
	val requestList = scenario("RequestList")
		.exec(
		Home.home,
		HostLogin.hostLogin,
		MyFlats.myFlats,
	...
	val requestListForbbiden = scenario("RequestListForbbiden")
		.exec(
		Home.home,
		TenantLogin.tenantLogin,
		RequestListForbbiden.requestListForbbiden)

	setUp(requestList.inject(atOnceUsers(10)),
		requestListForbbiden.inject(atOnceUsers(10)))
