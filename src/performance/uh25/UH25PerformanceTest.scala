...
class UH25PerformanceTest extends Simulation {
	...
	object UserPage { 
		val userPage = exec(http("UserPage")
			.get("/users/rbordessa0")
			.headers(headers_0))
		.pause(24)
	}
	
	object UserPageForbidden { 
		val userPageForbidden = exec(http("UserPageForbidden")
	...
	val loadUserPageForbidden = scenario("LoadUserPageForbidden")
		.exec(
		Home.home,
		UserPageForbidden.userPageForbidden)

	setUp(loadUserPage.inject(atOnceUsers(10)),
		loadUserPageForbidden.inject(atOnceUsers(10)))
