...
class UH12PerformanceTest extends Simulation {
	...
	object NewReview {
		val newReview = exec(http("NewReview")
			.get("/reviews/new?flatId=1")
			.headers(headers_0)
			.check(css("input[name=_csrf]", "value").saveAs("stoken")))
		.pause(8)
		.exec(http("NewReview")
			.post("/reviews/new?flatId=1")
			.headers(headers_3)
			.formParam("description", "Test review")
			.formParam("rate", "3")
			.formParam("creationDate", "23/05/2020")
			.formParam("reviewed", "1")
	...
	setUp(
		positiveScn.inject(rampUsers(2) during (10 seconds)),
		cantCreateReviewScn.inject(rampUsers(2) during (10 seconds))
