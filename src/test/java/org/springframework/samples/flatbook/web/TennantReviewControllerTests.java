...
@WebMvcTest(controllers = TennantReviewController.class,
includeFilters = {@ComponentScan.Filter(value = TennantFormatter.class, type = FilterType.ASSIGNABLE_TYPE), @ComponentScan.Filter(value = PersonFormatter.class, type = FilterType.ASSIGNABLE_TYPE)},
excludeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE, classes = WebSecurityConfigurer.class),
excludeAutoConfiguration= SecurityConfiguration.class)
public class TennantReviewControllerTests {

	private static final Integer TEST_TENNANTREVIEW_ID = 1;
    ...
    void setup() {
        ...
        TennantReview review = new TennantReview();
    	review.setCreationDate(creationDate);
    	review.setCreator(creator);
    	review.setDescription("description");
    	review.setRate(4);
    	review.setId(TEST_TENNANTREVIEW_ID);
    	
        Tennant notAllowed = new Tennant();
        notAllowed.setUsername(TEST_NOTALLOWED_USERNAME);

        given(this.tennantService.findTennantById(TEST_REVIEWED_USERNAME)).willReturn(reviewed);
        ...
        given(this.authoritiesService.findAuthorityById(TEST_NOTALLOWED_USERNAME)).willReturn(AuthoritiesType.TENNANT);
        ...
        given(this.tennantReviewService.findTennantReviewById(TEST_TENNANTREVIEW_ID)).willReturn(review);
    }

    @WithMockUser(value = TEST_CREATOR_USERNAME, roles = {"HOST"})
    @Test
    void testInitCreationForm() throws Exception {
        mockMvc.perform(get("/tennants/{tennantId}/reviews/new", TEST_REVIEWED_USERNAME))
            .andExpect(status().isOk())
            .andExpect(view().name("users/reviews/createOrUpdateTennantReviewForm"))
            .andExpect(model().attributeExists("tennantReview"));
    }

    @WithMockUser(value = TEST_CREATOR_USERNAME, roles = {"HOST"})
    @Test
    void testProcessCreationFormSuccess() throws Exception {
        mockMvc.perform(post("/tennants/{tennantId}/reviews/new", TEST_REVIEWED_USERNAME)
            .with(csrf())
    ...
    void testProcessCreationFormHasErrors() throws Exception {
        ...
        mockMvc.perform(post("/tennants/{tennantId}/reviews/new", TEST_REVIEWED_USERNAME)
        	...
        	.param("rate", "6"))
            .andExpect(status().isOk())
            .andExpect(model().attributeHasErrors("tennantReview"))
            .andExpect(model().attributeHasFieldErrors("tennantReview", "rate"))
            .andExpect(model().attributeHasFieldErrors("tennantReview", "creator"))
            .andExpect(view().name("users/reviews/createOrUpdateTennantReviewForm"));
    }
    ...
    @WithMockUser(value = TEST_CREATOR_USERNAME, roles = {"HOST"})
    @Test
    void testProcessTennantReviewRemovalSucess() throws Exception {
        mockMvc.perform(get("/tennants/{tennantId}/reviews/{tennantReviewId}/remove", TEST_REVIEWED_USERNAME, TEST_TENNANTREVIEW_ID))
            .andExpect(status().is3xxRedirection());
    }
    
    @WithMockUser(value = TEST_NOTALLOWED_USERNAME, roles = {"TENNANT"})
    @Test
    void testProcessTennantReviewRemovalNotAllowed() throws Exception {
        mockMvc.perform(get("/tennants/{tennantId}/reviews/{tennantReviewId}/remove", TEST_REVIEWED_USERNAME, TEST_TENNANTREVIEW_ID))
            .andExpect(status().is2xxSuccessful())
            .andExpect(view().name("exception"));
    }
